<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="mb-3">
    ...
    @endif
    <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Judul Resep</label>
        <input type="text" name="title" value="{{ old('title', $recipe->title) }}"
            class="w-full px-3 py-2 border border-gray-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition bg-white dark:bg-neutral-700 dark:text-gray-100 dark:placeholder-gray-500"
    ...
    <div class="mb-4">
        ...
        @if ($recipe->hero_image)
            <img src="{{ Storage::url($recipe->hero_image) }}" alt=""
                class="mt-2 rounded-lg h-16 w-16 object-cover bg-gray-100 dark:bg-neutral-700">
        @endif
    </div>

    <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Galeri Foto Tambahan</label>
        <div class="space-y-3">
            <input type="file" name="images[]" multiple id="gallery-input"
                class="w-full px-3 py-1.5 border border-gray-300 dark:border-neutral-600 rounded-lg text-sm dark:text-gray-300 dark:bg-neutral-700 file:mr-3 file:py-1 file:px-3 file:rounded-md file:border-0 file:bg-orange-50 dark:file:bg-orange-900/30 file:text-orange-600 dark:file:text-orange-400 file:font-medium hover:file:bg-orange-100 dark:hover:file:bg-orange-900/50 cursor-pointer">
            <p class="text-xs text-gray-500 dark:text-gray-400">Bisa pilih banyak foto sekaligus. Kapasitas timbun foto
                tak terbatas!</p>
            ...
            <div id="gallery-preview" class="flex flex-wrap gap-3">
                @if ($recipe->images->count() > 0)
                    @foreach ($recipe->images as $image)
                        <div
                            class="relative w-20 h-20 rounded-lg overflow-hidden border border-gray-200 dark:border-neutral-600 group">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Waktu Persiapan
                (menit)</label>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Waktu Masak (menit)</label>
            <input type="number" name="cook_time" value="{{ old('cook_time', $recipe->cook_time) }}"
                class="w-full px-3 py-2 border border-gray-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition bg-white dark:bg-neutral-700 dark:text-gray-100 dark:placeholder-gray-500"
                placeholder="Contoh: 30">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Porsi (orang)</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
        <div>
            ...
            <select name="difficulty"
                class="w-full px-3 py-2 border border-gray-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition bg-white dark:bg-neutral-700 dark:text-gray-100">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Jenis Diet</label>
            <input type="text" name="diet" value="{{ old('diet', $recipe->diet) }}"
                class="w-full px-3 py-2 border border-gray-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition bg-white dark:bg-neutral-700 dark:text-gray-100 dark:placeholder-gray-500"
                placeholder="Contoh: vegetarian, halal">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Asal Masakan</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Bahan-bahan</label>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cara Membuat</label>
    ...
    <div class="mt-4">
        <label class="flex items-center gap-2 mb-3 font-bold text-gray-500 dark:text-gray-400 text-sm">
        ...
        <div class="flex flex-wrap gap-2">
            @foreach ($tags as $tag)
                <label class="cursor-pointer relative select-none group">
                    ...
                    <div
                        class="px-3 py-2 rounded-full border-2 border-gray-100 dark:border-neutral-600 bg-white dark:bg-neutral-700 text-gray-500 dark:text-gray-300 text-sm font-bold transition-all duration-300
        ...
        if (files) {
            Array.from(files).forEach(file => {
                ...
                reader.onload = function(e) {
                    ...
                    div.className =
                        'relative w-20 h-20 rounded-lg overflow-hidden border border-gray-200 group new-preview';
                    div.innerHTML =
